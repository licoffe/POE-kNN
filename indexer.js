...
var downloadInterval = 750; // Time between downloads in seconds
 ...
 *
 * @params Second amount to convert
 ...
 * @params item from stash API, callback
 ...
 *
 * Download chunk from POE stash API using compression, parse and check if next 
 * chunk is available. If yes, recurse with next chunk ID.
...
var downloadChunk = function( chunkID, collection, db, callback ) {
    ...
    var download = function( chunkID ) {
        ...
        dnscache.lookup( "www.pathofexile.com", function( err, result ) {
            ...
            request({ "url": "http://" + result + "/api/public-stash-tabs?id=" + chunkID, "gzip": true },
    ...
    /**
     * Process chunk content and store it inside the DB
     *
     * long description
     * @params parsed data (JSON)
     * @return Nothing
     */
            ...
            if ( err ) {
                ...
                chunk_collection.insert(
                    ...
                    async.each( data.stashes, function( stash, callbackStash ) {
                        // If the stash is updated, the account is online
                        db.createCollection( 'online_status', function( err, onlineCollection ) {
                            ...
                            // Update account name online status
                        ...
                        // Get previously stored stash contents associated to 
                        // this stash ID
                        getStashByID( db, stash.id, function( results ) {
                            ...
                            // If the stash already exists
                                ...
                                // Check which item has been removed, added or kept
    ...
    // Called after data parsing and storing
    var done = function( data ) {
        var chunkProcessingTime = Date.now() - begin;
        var delay  = ( downloadInterval - chunkProcessingTime ) > 0 ? ( downloadInterval - chunkProcessingTime ) : 0;
        ...
        if ( interrupt ) {
            ...
            // Sleep n seconds and call the script on the next chunk ID
            ...
            logger.log( "Sleeping " + delay + "ms", script_name );
    ...
    connectToDB( function( db ) {
        ...
        db.createCollection( stashCollection, function( err, collection ) {
            ...
            Promise.all(createIndexs(indexFields)).then(value => {
                // If a change id was passed as arg
                if ( process.argv.length > 2 && !debug ) {
                    downloadChunk( process.argv[2], collection, db, downloadChunk );
                // Otherwise
                } else {
